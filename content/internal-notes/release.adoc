...
image::release-branching.png[Release branching]
...
===== Automated back-porting

There is an automation in place to ease the process of back-porting changes.
The automation works on a per-PR basis.
When you want to back-port a change, mark the original PR (targeting `main` branch) with `backport` label.
The back-porting process will start depending on the merge status of the original PR:

* In case the PR is already merged when adding the label, the process will start right away.
* If the PR is yet to be merged, the back-porting will happen once the PR is merged.

The automation will conduct all the necessary steps enumerated in the following section, and additionally will copy the labels from the original PR to the backport one.
The team member that is carrying away the process should manually merge the backport PR after ensuring everything is correct.

In case of a merge conflict, a draft PR will be created instead of a normal PR.
A team member is expected to checkout the branch created for the backport PR and resolve the conflict manually.
Then, he needs to remove the draft status of the PR and merge it.

[NOTE]
====
If there is a merge conflict and if resolving the conflict is too complex, the team might decide to drop the back-port.
In that case, don't forget to also remove the `released-as-hotfix` label from the original PR.
====

===== Manual back-porting

If for some reason you need to create the backport manually, follow these steps:
