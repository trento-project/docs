...
main:: Contains the newly developed features and is kept stable at all
times by using Pull Requests, Peer Reviews and automated CI checks
before any new code integration.

release:: Branch contains the code from `main` that is ready for
release plus potentially small custom fixes.

We support two types of releases:

Full releases:: Made by cross-merging `main` and `release` branches
