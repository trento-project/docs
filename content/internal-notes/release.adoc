...
There is an automation in place to ease the process of backporting changes.
The automation works on a per-PR basis.
When you want to backport a change, mark the original PR (targeting `main` branch) with `backport` label.
The backporting process depends on the merge status of the original PR:

* In case the PR is already merged when adding the label, the process starts immediately.
* If the PR is yet to be merged, the backporting occurs after the PR is merged.

The automation performs the necessary steps listed in the following section and copies all labels from the original PR to the backport PR.
The team member handling the process should review the backport PR and merge it manually.

In case of a merge conflict, a draft PR is created instead of a normal PR.
A team member is expected to checkout the branch created for the backport PR and resolve the conflict manually.
Then, the team member needs to remove the draft status of the PR and merge it.

[NOTE]
====
If there is a merge conflict and if resolving the conflict is too complex, the team might decide to drop the backport.
In that case, do not forget to also remove the `released-as-hotfix` label from the original PR.
====

===== Manual backporting

If you need to create the backport manually, follow these steps:
