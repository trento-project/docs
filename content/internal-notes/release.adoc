...
. allows for quick releases when the need arises
. allows for simple versioning for nightly builds
...
feature-complete, a need for more branches arises.

Thus, Trento settle for semi-linear approach to branching and
release. This means a Trento repository has two long-running branches
...
shared by both branches because once one of the branches is merged
into the other, the reverse operation is a fast-forward merge. They
contain all the new features developed on `main` since the last full
release. They happen on a cadence of roughly 6 months.

Hotfix releases allows for quick reaction on unforeseen problems with
...
will trigger the following automated steps:

- automatically generate CHANGELOG entries from the names of the PRs
  merged into the respective branch, new commit is made on the on that
  same branch for this by the CI bot;
...
when the project is ready to make a new full release:
...
From then on, the CI will finish the process for you.
...
NOTE: Ideally, adding that label should not be needed. However, it's
caused by a limitation in the changelog generator we currently use.
...
Again, the CI will take it from here.
