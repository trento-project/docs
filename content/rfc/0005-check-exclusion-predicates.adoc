== 5. Host-level Exclusion Predicates for Check Executions
...
3. Reports excluded pairs in the execution result with a clear "excluded" status, distinct from failed / skipped, so the UI can render it.
...
|Wanda
|Filter (host, check) pairs before sending `FactsGatheringRequested` to agents. Report excluded pairs in the execution result with a distinct "excluded" status.
