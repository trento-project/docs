...
* https://github.com/agentjido/jido[agentjido/jido] not langchain oriented, though.
* https://github.com/sagents-ai/sagents[sagents] 
...
Langchain https://github.com/brainlid/langchain[library] itself provides a set of primitives and tools to integrate with LLMs and build AI agents. We need to implement the actual agent logic ourselves, using the LangChain library as a foundation.

In this regard, the https://github.com/brainlid[same author] of the LangChain Elixir library has also implemented a higher level framework called https://github.com/sagents-ai/sagents[Sagents] which provides more batteries included features and abstractions to build AI agents. Take a look also at its https://github.com/sagents-ai/agents_demo[demo application].

It's worth noting that:
* we can introduce the more advanced features (like HITL) that Sagents provides incrementally
* Sagents provides a generator/scaffolding tool to create the basic building blocks for the agent
* Sagents natively addresses LiveView integration, however we can revisit it to suit our needs and work with Channels
* we successfully had it working in an AG-UI+Assistant-UI+websocket setup in our iterations, meaning that it is possible to use it in the way we want to use it
...
Given the above, the proposed solution is TBD...
...
* There is work in progress in the AG-UI ecosystem to support the Elixir https://github.com/ag-ui-protocol/ag-ui/pull/1046[ag-ui/pull/1046] and https://github.com/ag-ui-protocol/ag-ui/pull/1293[ag-ui/pull/1293]
* We can implement only the parts of the protocol that are relevant to us
====
