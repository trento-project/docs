== 3. Grafana Alloy Integration for SLES 16 System Monitoring

[width="100%",cols="<18%,<82%",]
|===
|Feature Name | Grafana Alloy Integration for SLES 16 System Monitoring
|Start Date | Dec 22nd, 2025
|Category | Architecture, Monitoring
|PR | https://github.com/trento-project/docs/pull/126[#126]
|===
...
|No
|The Prometheus remote write endpoint URL (used in push mode with Alloy)
...
# Prometheus endpoint URL (used in push mode with Alloy)
...
For `alloy`-based systems, `prometheus-exporter-mode` must be set to `push` and `prometheus-url` must point to the Prometheus remote write endpoint. The `generate alloy` command reads these settings from the configuration file. Authentication settings are required for production deployments (bearer token is the default):
...
prometheus-exporter-mode: push  # push for Alloy, pull for node_exporter
prometheus-url: https://prometheus-url/api/v1/write  # reverse-proxied via nginx/Ingress
prometheus-exporter-name: grafana_alloy  # optional, defaults to grafana_alloy in push mode, node_exporter in pull mode
...
For `node_exporter`-based systems (SLES 15), use pull mode configuration:
...
# Prometheus exporter configuration for pull mode (default)
prometheus-exporter-mode: pull  # or omit this line, defaults to pull
prometheus-node-exporter-target: http://localhost:9100/metrics  # optional, auto-discovered if not specified
prometheus-exporter-name: node_exporter  # optional, defaults to node_exporter in pull mode
----

===== Backward Compatibility

To maintain backward compatibility with existing installations:

* `prometheus-exporter-mode` defaults to `pull` if not specified
* `prometheus-exporter-name` defaults to:
  - `node_exporter` in pull mode
  - `grafana_alloy` in push mode
* `prometheus-node-exporter-target` is used in pull mode (where Prometheus pulls from node_exporter)
* `prometheus-url` is used in push mode (where Alloy pushes to Prometheus)
* If the mode is `pull`, the `prometheus-url` flag is ignored
* If the mode is `push`, the `prometheus-node-exporter-target` flag is ignored

The agent will fail to start with a clear error message if:
* `prometheus-exporter-mode` is `push` but `prometheus-url` is not provided
* The configuration contains ambiguous or conflicting settings
...
===== Alternative Naming: Unified URL Parameter

An alternative configuration design was considered using a single `prometheus-exporter-url` parameter for both push and pull modes instead of separate `prometheus-url` and `prometheus-node-exporter-target` flags. This approach would use `--prometheus-exporter-url` whose meaning would be context-dependent: pointing to the Prometheus remote write endpoint in push mode, or to the node_exporter target in pull mode.
...
Specifically, the `prometheus_mode` field is added providing the mode the agent is expecting to operate; it is set explicitly via the `prometheus-exporter-mode` configuration key (defaulting to `pull`). Trento server can interpret a missing field as `pull` mode, and that is safe for existing installations.

The field `prometheus_targets` is kept for backward compatibility, too. The key in the map is determined by the `prometheus-exporter-name` configuration (defaulting to `node_exporter` in pull mode or `grafana_alloy` in push mode), and the value is the target address:
* In pull mode: configured via `prometheus-node-exporter-target` (or auto-discovered from the host's lowest IPv4 address if not specified)
* In push mode: configured via `prometheus-url`
...
{
    [...]
    "prometheus_targets": { "<prometheus-exporter-name>": "<target-address>" },
...
|Trento Agent
|Add `prometheus-exporter-mode` (default: `pull`), `prometheus-exporter-name` (default: `node_exporter` in pull mode, `grafana_alloy` in push mode), `prometheus-url` (for push mode), and `prometheus-node-exporter-target` (for pull mode) configuration options. The agent uses different URL flags depending on the mode, ignoring the non-relevant one.
