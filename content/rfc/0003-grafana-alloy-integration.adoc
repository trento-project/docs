...
:author: Trento Engineering Team
...
type AlloyConfigData struct {
    ...
    // Authentication
    AuthMethod      string // "none", "basic", "bearer", or "mtls"
    AuthUsername    string // for basic auth
    AuthPassword    string // for basic auth
    AuthBearerToken string // for bearer auth
    
    // TLS
    TLSCACert     string // CA certificate path
    TLSClientCert string // Client certificate path (for mTLS)
    TLSClientKey  string // Client key path (for mTLS)
}

var generateAlloyCmd = &cobra.Command{
    Use:   "alloy",
    Short: "Generate Alloy configuration for Trento metrics collection",
    Long: `Generate a Grafana Alloy configuration file for collecting system metrics
and pushing them to the Trento server's Prometheus endpoint.

The configuration is printed to stdout. Pipe the output to your preferred 
location or use shell redirection to save to a file.

Options can be provided via command-line flags, environment variables 
(TRENTO_ prefix), or the configuration file.`,
    RunE: runGenerateAlloy,
}

func runGenerateAlloy(cmd *cobra.Command, args []string) error {
    prometheusURL := viper.GetString("prometheus-url")
    scrapeInterval := viper.GetString("scrape-interval")
    authMethod := viper.GetString("prometheus-auth")
    
    // Agent ID is determined automatically, same as in 'start' command
    agentID, err := getAgentID()
    if err != nil {
        return fmt.Errorf("failed to determine agent ID: %w", err)
    }
    
    // Allow override for development/testing (hidden flag)
    if forceID := viper.GetString("force-agent-id"); forceID != "" {
        agentID = forceID
    }
    ...
    // Validate and populate auth configuration based on method
    switch authMethod {
    case "none":
        // No additional configuration needed
    case "basic":
        data.AuthUsername = viper.GetString("prometheus-auth-username")
        data.AuthPassword = viper.GetString("prometheus-auth-password")
        if data.AuthUsername == "" || data.AuthPassword == "" {
            return fmt.Errorf("prometheus-auth-username and prometheus-auth-password are required for basic authentication")
        }
    case "bearer":
        data.AuthBearerToken = viper.GetString("prometheus-auth-bearer-token")
        if data.AuthBearerToken == "" {
            return fmt.Errorf("prometheus-auth-bearer-token is required for bearer authentication")
        }
    case "mtls":
        data.TLSClientCert = viper.GetString("prometheus-tls-client-cert")
        data.TLSClientKey = viper.GetString("prometheus-tls-client-key")
        if data.TLSClientCert == "" || data.TLSClientKey == "" {
            return fmt.Errorf("prometheus-tls-client-cert and prometheus-tls-client-key are required for mTLS authentication")
        }
    default:
        return fmt.Errorf("unsupported authentication method: %s (supported: none, basic, bearer, mtls)", authMethod)
    }
    
    tmpl, err := template.New("alloy").Parse(alloyTemplate)
    if err != nil {
...
func init() {
    generateCmd.AddCommand(generateAlloyCmd)
    
    generateAlloyCmd.Flags().String("prometheus-url", "", "Prometheus remote write URL")
    generateAlloyCmd.Flags().String("prometheus-auth", "bearer", "Authentication method: none, basic, bearer, mtls")
    generateAlloyCmd.Flags().String("prometheus-auth-username", "", "Username for basic authentication")
    generateAlloyCmd.Flags().String("prometheus-auth-password", "", "Password for basic authentication")
    generateAlloyCmd.Flags().String("prometheus-auth-bearer-token", "", "Bearer token for bearer authentication")
    generateAlloyCmd.Flags().String("prometheus-tls-ca-cert", "", "Path to CA certificate for TLS verification")
    generateAlloyCmd.Flags().String("prometheus-tls-client-cert", "", "Path to client certificate for mTLS")
    generateAlloyCmd.Flags().String("prometheus-tls-client-key", "", "Path to client key for mTLS")
    generateAlloyCmd.Flags().String("scrape-interval", "15s", "Metrics scrape interval")
    generateAlloyCmd.Flags().String("force-agent-id", "", "Override agent ID (development only)")
    generateAlloyCmd.Flags().MarkHidden("force-agent-id")
    
    // Bind flags to viper for config file support
    viper.BindPFlag("prometheus-url", generateAlloyCmd.Flags().Lookup("prometheus-url"))
    viper.BindPFlag("prometheus-auth", generateAlloyCmd.Flags().Lookup("prometheus-auth"))
    viper.BindPFlag("prometheus-auth-username", generateAlloyCmd.Flags().Lookup("prometheus-auth-username"))
    viper.BindPFlag("prometheus-auth-password", generateAlloyCmd.Flags().Lookup("prometheus-auth-password"))
    viper.BindPFlag("prometheus-auth-bearer-token", generateAlloyCmd.Flags().Lookup("prometheus-auth-bearer-token"))
    viper.BindPFlag("prometheus-tls-ca-cert", generateAlloyCmd.Flags().Lookup("prometheus-tls-ca-cert"))
    viper.BindPFlag("prometheus-tls-client-cert", generateAlloyCmd.Flags().Lookup("prometheus-tls-client-cert"))
    viper.BindPFlag("prometheus-tls-client-key", generateAlloyCmd.Flags().Lookup("prometheus-tls-client-key"))
    viper.BindPFlag("scrape-interval", generateAlloyCmd.Flags().Lookup("scrape-interval"))
    viper.BindPFlag("force-agent-id", generateAlloyCmd.Flags().Lookup("force-agent-id"))
}
----
