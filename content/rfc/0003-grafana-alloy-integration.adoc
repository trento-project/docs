...
For `alloy`-based systems, `prometheus-mode` must be set to `push` and `prometheus-url` must point to the Prometheus remote write endpoint. The `generate alloy` command reads these settings from the configuration file. Authentication settings are required for production deployments (bearer token is the default):
...
prometheus-mode: push  # push for Alloy, pull for node_exporter
...
prometheus-mode: pull  # or omit this line, defaults to pull
...
* `prometheus-mode` defaults to `pull` if not specified
...
* `prometheus-mode` is `push` but `prometheus-url` is not provided
...
image::alloy-status-pill.png[Status Pill Example]
...
|Trento Agent
|Add `prometheus-mode` (default: `pull`), `prometheus-exporter-name` (default: `node_exporter` in pull mode, `grafana_alloy` in push mode), `prometheus-url` (for push mode), and `prometheus-node-exporter-target` (for pull mode) configuration options. The agent uses different URL flags depending on the mode, ignoring the non-relevant one.
