...
Add a single new field `host_data` on `Target`, typed as `map<string, google.protobuf.Value>`. The map is free-form: there are no typed fields, no reserved keys, and no schema-level contract on the set of keys Web emits. Values use `google.protobuf.Value` so scalars, lists and nested structs are all representable without breaking the wire format. This mirrors the existing `ExecutionRequested.env` field (same package, same import), so we do not introduce a new convention.
...
message Target {
    ...
    // Free-form host metadata known to Trento Web at request time.
...
Wanda exposes `Target.host_data` to predicates as a single `host` object, where each entry is accessed by key: `host["provider"]`, `host["is_majority_maker"]`, `host["cluster_role"]`. Absent keys return `()` so predicates can guard with the usual Rhai patterns.
...
|Contracts
|Add `map<string, google.protobuf.Value> host_data = 3;` to `Target` in `target.proto`. Free-form: no typed fields, no reserved keys. Purely additive; producer/consumer alignment on key names lives outside the proto.
