...
Options in this regard are:
 ...
 . make the introspection endpoint internal only
 ...
 * we get rid of wanda/web interdependency on `ACCESS_TOKEN_ENC_SECRET`, however we exchange it with a shared `username/password` or `client_id/client_secret` pair
...
===== *Option 4: RPC calls from Wanda to Web*
PROs:

 * we get rid of wanda/web interdependency on `ACCESS_TOKEN_ENC_SECRET`, however we exchange it with the shared erlang cookie
 * no new authn mechanism needed
 * web and wanda become clustered elixir applications

CONs:

 * deviates from the current agreement of having services communicate via HTTP APIs
 * introduces an integration paradigm we might want discuss deeper upon before adopting it

===== *Option 5: Internal only endpoint*
PROs:
 
 * we do not need to design and maintain another authn mechanism
 * we get rid of wanda/web interdependency on `ACCESS_TOKEN_ENC_SECRET`
 * we take time to properly design and introduce OAuth terminology and flows at a later stage

CONs:

 * if the proxy in front of services is not configured properly, the endpoint could be exposed publicly (ie the customer does not use our provided proxy configuration)

Proposed option is *Option 5: Internal only endpoint* so that:

 * we can take time to properly design and introduce OAuth terminology and flows at a later stage
 * contextually get rid of wanda/web interdependency on `ACCESS_TOKEN_ENC_SECRET`
 * we do not need to design and maintain another authn mechanism
