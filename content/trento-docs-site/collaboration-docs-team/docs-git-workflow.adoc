...
Ownership of the repository and its release branches is shared in practice, but not every branch has the same role:
...
* `latest` is a shared branch because it is used to build the official user-facing documentation

Because `latest` is used for the official user-facing documentation, changes under `trento/` that are promoted to `latest` should be reviewed by a member of the documentation team.
