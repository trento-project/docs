...
Users and user managers may also set a preferred timezone for the
account from the *Create User* and *Edit User* views. Individual users
can configure their own preference later in the *Profile* view. The
web console displays dates and times according to each user's preferred
timezone for presentation; if no preference is set, UTC is used. This
preference affects only how timestamps are presented in the UI —
backend storage, API payloads, logs and scheduled tasks continue to use
UTC.
