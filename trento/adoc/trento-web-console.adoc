...
.Dashboard with the dash indicators
[#fig-trento-web-dash]
image::trento-web-dash.png[{trento} dashboard showing the passing, warning and critical health boxes together with dash indicators,scaledwidth=100.0%]
...
|{hana} databases
|Overall status of {hana} instances

|{sap} systems
|Overall status of {sap} instances, aggregated health of the {hana} database
...
Overall status of {hana} database instances / {sap} instances::
...
[#fig-trento-web-home]
image::trento-web-home.png[{trento} dashboard showing the Passing, Warning and Critical health boxes above the list of SAP systems,scaledwidth=100.0%]
...
instances of the system. Database instances are not taken into consideration here.
...
* *Warning banner.* The _Details_ views of affected hosts, clusters, {hana} databases and {sap} systems display a warning banner.
...
.Stale health icons
[#fig-trento-stale-icons]
image::trento-stale-icons.png[Health icons with a clock overlay,scaledwidth=100.0%]

.Stale warning banner
[#fig-trento-stale-banner]
image::trento-stale-banner.png[A banner with warning about agent not reporting,scaledwidth=100.0%]
...
. In the {tr_web} console *Dashboard*, click the health box that matches the value you want to investigate, for example *Critical*. The dashboard now lists only the systems with a layer matching health value.
...
This happens when {trento} discovers an {sap} workload on the host, and either `{saptune}` is not installed, the installed version is older than 3.1, or no tuning solution is applied to the host. If the host shows a _critical_ value instead, it means the tuning status is not compliant. In this last case, run `saptune note verify` on the host for further details.
