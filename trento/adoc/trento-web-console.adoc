...
This happens if `{saptune}` is not installed, the installed version is older than 3.1, no tuning solution is applied to the host, or tuning status is not compliant. In this last case, run `saptune note verify` on the host for further details.
