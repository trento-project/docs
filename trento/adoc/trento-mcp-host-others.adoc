...
The {tr_mcp} can be integrated with any client application that supports the Model Context Protocol. This makes it possible to interact with the {trento} API and execute tools defined in the OpenAPI specification through your preferred AI assistant or development tool.
...
Once you have your {trserver} installation ready with the {tr_mcp} URL and API token, you can configure the MCP Server client. The examples below show JSON configuration format used by most MCP Server clients, including VS Code, Claude Desktop, and others.
