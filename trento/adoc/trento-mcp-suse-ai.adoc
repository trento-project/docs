...
===== Adding the {tr_mcp} to Open Web UI
...
.Open Web UI - Add the {tr_mcp} as a tool
...
. In the *Tools* or *MCP Servers* section, enable/select `{tr_mcp}` for this model. This makes the {trento} tools available to any user of that model in Open Web UI.
...
.Open Web UI - Enable {tr_mcp} for a model
image::owui-add-model.png[Enable {tr_mcp} for model,width=600]

===== Using the {tr_mcp}

Once the {tr_mcp} is configured and enabled for your model, you can start using it to interact with your SAP systems through natural language conversations.
...
.Open Web UI - Using {tr_mcp} in conversations
image::example-suseai.png[Using {tr_mcp} in Open Web UI,width=900]
