...
The MCP Server will automatically connect to the {trweb} and Wanda internal services within the {k8s} cluster.
...
helm upgrade --install trento-server oci://registry.suse.com/trento/trento-server \
  ...
  --set trento-mcp-server.ingress.hosts[0].paths[0].path=/mcp
----
====

The MCP Server endpoint will be: `https://TRENTO_SERVER_HOSTNAME/mcp`
