...
Search for Traefik link:https://github.com/search?q=repo%3Atrento-project%2Fhelm-charts+traefik&type=code[specific usage scenarios on GitHub]. In case another ingress controller is used, adapt accordingly.
====
...
[NOTE]
====
The examples in this section do not specify a {k8s} namespace for simplicity. By default, {helm} installs to the `default` namespace.
