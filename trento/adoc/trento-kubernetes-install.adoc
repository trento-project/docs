...
// ==== Installing {tr_mcp} (Optional)
// 
// The {tr_mcp} provides AI-assisted management capabilities for your SAP landscape by exposing {trento} functionality through the Model Context Protocol (MCP). This optional component enables integration with AI tools and assistants.
// 
// [NOTE]
// ====
// The {tr_mcp} is an optional component. The core {trserver} functionality works independently without it.
// 
...
// ===== Prerequisites
// 
// Before installing the {tr_mcp}, ensure you have:
// 
...
// 
// The {tr_mcp} requires minimal configuration, but needs to know how to reach your Trento Server. The key configuration parameters are:
// 
