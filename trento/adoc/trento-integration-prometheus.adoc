...
If you already have an existing {prometheus} Server that you want to use with {trserver}, you must set `CHARTS_ENABLED=true` and `PROMETHEUS_URL` pointing to the right address and port in the {trweb} configuration file. You must restart restart the {trweb} service to enable the changes.
