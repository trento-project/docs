...
*** For a {k8s} deployment, from the MCP client to the {trserver} ingress at TCP/80 or TCP/443.
*** For a systemd deployment, from the MCP client to the proxy at TCP/443; if the {tr_mcp} is exposed directly, to TCP/{tr_mcp_default_port}.
