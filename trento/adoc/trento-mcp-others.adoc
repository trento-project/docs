...
==== Integrating the {tr_mcp} with other MCP Hosts
:revdate: 2025-10-31

The {tr_mcp} can be integrated with any client application that supports the Model Context Protocol (MCP). This allows you to interact with the {trento} API and execute tools defined in the OpenAPI specification through your preferred AI assistant or development tool.
...
** Follow the <<sec-trento-installing-trentoserver>> to deploy {trserver} and enable the {tr_mcp}.
** Note the {tr_mcp} endpoint URL (e.g., `https://trento.example.com/mcp-server-trento/mcp`).
...
Once you have your {trento} installation ready with the {tr_mcp} URL and API token, you can configure your MCP Host. The examples below show JSON configuration format, which is used by most MCP Hosts, including VS Code, Claude Desktop, and others.
...
[NOTE]
====
Replace `https://trento.example.com/mcp-server-trento/mcp` with your actual {tr_mcp} endpoint URL from your installation.
====
...
* Replace `https://trento.example.com/mcp-server-trento/mcp` with the actual URL where your {tr_mcp} is accessible:
  ...
  ** If using Server-Sent Events (SSE) transport instead of the default streamable transport, change the path from `/mcp` to `/sse` - The transport type is configured on the {tr_mcp} using the `TRANSPORT` configuration key or `--transport` flag (see <<sec-trento-mcp-config>>).
...
If you encounter issues connecting your MCP Host to the {tr_mcp}:

. *Connection Errors*:
** Verify that the {tr_mcp} URL is correct and accessible from your machine.
...
How you use the {tr_mcp} depends on the specific MCP Host application you're working with. The MCP ecosystem is rapidly evolving, with new hosts and improved user experiences being introduced regularly.

====== Enable the Server and Expose Tools in Your MCP Host

[arabic]
. Ensure the {tr_mcp} is configured in your host (see the configuration examples above).
...
.. Some hosts let you choose which MCP servers are available per model or project - Make sure the *{tr_mcp}* is enabled for the target model/session.
...
To see the {tr_mcp} in action, watch link:https://www.youtube.com/watch?v=7kDVc3YUR-U[this demonstration video].

Find below a few example screenshots from different MCP Hosts using the {tr_mcp}:

====== Example: Claude Desktop

The Claude Desktop app can attach the {tr_mcp} and expose its tools directly in conversations. After configuring your server in Claude's MCP settings, start a new chat and look for the available tools.

.Claude Desktop using {tr_mcp}
image::example-claude.png[Claude Desktop using {tr_mcp},width=900]

====== Example: Visual Studio Code with GitHub Copilot

VS Code lets you register external MCP servers and make their tools available to Copilot Chat. Once configured, start a chat and pick the {tr_mcp} tools to run common operations.

.Visual Studio Code with GitHub Copilot using {tr_mcp}
image::example-copilot.png[VS Code Copilot using {tr_mcp},width=900]
