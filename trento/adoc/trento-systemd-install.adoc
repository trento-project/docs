...
[NOTE]
====
{prometheus} is optional, but you must configure the chart display setting after installing {trento}.
After the installation, edit the web configuration file in +/etc/trento/trento-web+

* To disable charts when not using {prometheus}, set +CHARTS_ENABLED+ to +false+.
* To enable charts using an existing {prometheus}, installation, set +CHARTS_ENABLED+ to +true+.
====

====== Choose a {prometheus} Setup Option

* <<prometheus-existing-installation, Use an existing Prometheus installation>>  
* <<prometheus-unsupported-packagehub, Install Prometheus from the *unsupported* PackageHub repository for {sles4sap} 15.4 and higher>>
* <<prometheus-supported-packagehub, Install Prometheus on {sles4sap} 16.0>>
...
If you already have an https://prometheus.io/docs/prometheus/latest/installation/[existing Prometheus server], set the +PROMETHEUS_URL+ environment variable to point to your {prometheus} server URL after the installation of trento-web.

Refer <<prometheus-config, for an example of {prometheus} configuration>>.

---

[[prometheus-unsupported-packagehub]]
====== Install {prometheus} from PackageHub for {sles4sap} 15.x
...
. Enable the PackageHub repository (replace `x.x` with the version of your operating system, for example `15.7`):
+
[source,bash]
----
...
zypper refresh
----
Proceed with the same steps as in <<prometheus-supported-packagehub, Install Prometheus on {sles4sap} 16.0>>

[[prometheus-supported-packagehub]]
====== Install supported {prometheus} for {sles4sap} 16.0
...
[NOTE]
====
Only available for SLE 15 SP4-SP7, for SLE 16.0 use alternatives or the
xref:option-1-creating-a-self-signed-certificate[Self-signed certificate option].
====
