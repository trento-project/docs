...
Refer to <<prometheus-packagehub>> for an example of {prometheus} configuration.
...
. Enable the PackageHub repository (replace `x.x` with the version of your operating system, for example `15.7`), if {sles4sap} 15 is used:
+
[source,bash]
----
