...
SAP Applications. For installations on Service Packs other than the current one,
update the repository address as indicated in the respective notes provided
...
server {
    ...
    # Redirect openapi requests from Wanda doc portal
    location ~ ^/api/(v[0-9]+|latest|all|unversioned)/openapi$ {
        if ($http_referer ~* "^https?://[^/]+/wanda/") {
            return 302 /wanda$request_uri;
        }

        # If not from wanda, proceed to TrentoWeb
    ...
    # Wanda rule for checks, groups, etc.
    location ~ ^/(api/checks|api/v[0-9]+/checks|api/groups|api/v[0-9]+/groups)/  {
        allow all;
    ...
    # Wanda openapi endpoints for all versions
    location ~ ^/wanda/api/(v[0-9]+|latest|all|unversioned)/openapi {
        allow all;
        ...
        proxy_pass http://localhost:4001/api/$1/openapi;
        proxy_redirect /api/$1/openapi /wanda/api/$1/openapi;
    }

    # Wanda doc endpoint
    location /wanda/api/doc {
        allow all;
        ...
        proxy_pass http://localhost:4001/api/doc;
        proxy_redirect /api/doc /wanda/api/doc;
    }
