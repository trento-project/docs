...
[NOTE]
====
At this stage, `trento-web` is running and can be accessed directly using its host IP address and configured port.

If `firewalld` is enabled, open the required port to allow direct access:
...
Alternatively, you can proceed with setting up NGINX to expose {trento} through a public HTTPS URL secured with certificates.
====
