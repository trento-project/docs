...
{trento} is part of {sles4sap}. {trento}'s two main components have the following product lifecycles:

// Variable List for tragent
{tragent}::
+
Delivery mechanism::: RPM package for {sles4sap} 15 SP4 and newer, and {sles4sap} 16.0.
...
{trserver}::
+
Delivery mechanisms::: A set of container images from the {suse} public registry together with a Helm chart that facilitates their installation or a set of RPM packages for {sles4sap} 15 SP4 and newer, and {sles4sap} 16.0.
...
systemd deployments::: Supported in {sles4sap} 15 SP4 and newer, and {sles4sap} 16.0 on x86_64 and ppc64le architectures.
