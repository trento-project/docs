...
This section provides an overview of how to configure the {tr_mcp}.

* <<sec-trento-mcp-kubernetes-config,{k8s} configuration>>
* <<sec-trento-mcp-systemd-config, systemd RPM deployment >>
...
===== Configuration Sources Kubernetes
...
{tr_mcp} allows you to override configuration settings using command-line flags for temporary changes. These overrides are not persistent and are lost when the process stops or the system is rebooted. To make configuration changes permanent for the systemd service, update `/etc/trento/mcp-server-trento`.
...
The {tr_mcp} includes built-in health check endpoints for systemd and Kubernetes integration.
