...
=== Configure {tr_mcp} with the command-line
:revdate: 2025-11-24
...
|`Authorization`
|Header name used to pass the Trento API key to the MCP server.
...
|`TRENTO_MCP_PORT`
|`5000`
|Port where the MCP server runs.
...
[source,console]
----
mcp-server-trento --port 9000 --verbosity debug --trento-url {tr_server_url_https}
----
...
[source,console]
----
mcp-server-trento --trento-url {tr_server_url_https}--autodiscovery-paths /api/v1/openapi,/wanda/api/v1/openapi
----
