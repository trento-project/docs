...
* <<sec-trento-mcp-systemd-config,Configuration in an systemd deployment>>
...
[[sec-trento-mcp-k3s-health-helm]]
===== Enable Health Checks with {helm} on a {k8s} deployment

Enable health checks when deploying on {k8s} with {helm}:

[source,console,subs="attributes"]
----
helm upgrade \
  --install trento-server oci://registry.suse.com/trento/trento-server \
  --set trento-web.trentoWeb.origin=TRENTO_SERVER_HOSTNAME \
  --set trento-web.adminUser.password=ADMIN_PASSWORD \
  --set trento-mcp-server.enabled=true \
  --set TRENTO_MCP_ENABLE_HEALTH_CHECK=true \
  --set TRENTO_MCP_HEALTH_PORT={tr_mcp_health_port}
----

The health port is internal to the {k8s} cluster. To reach it from the host running {k8s}, forward the Pod port. Replace `NAMESPACE` with your target namespace (Helm defaults to `default`).
...
kubectl port-forward --namespace NAMESPACE \
  ...
  8080:8080 &
----
With the port forward active, test the endpoints in <<sec-trento-mcp-health-testing,Testing Health Endpoints>>.
...
.. For systemd deployments, use `{tr_mcp_binary} --help`
.. For {k8s} deployments, run {helm} with `--render-subchart-notes` to view the rendered {tr_mcp} settings
